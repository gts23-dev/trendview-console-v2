...
function DataGridPagination(props: DataGridPaginationProps) {
  ...
  // Metronic 원본과 다른 점: sizes 기본값을 없애 쪽당 개수 선택기를 옵션으로
  // 돌렸다. 목록마다 per_page가 서버 요청에 고정돼 있어 기본은 숨긴다.
  ...
  return (
    ...
    >
      <div className="order-2 flex flex-wrap items-center space-x-2.5 pb-2.5 sm:order-1 sm:pb-0">
        {isLoading ? (
          mergedProps?.sizes?.length ? (
            mergedProps.sizesSkeleton
          ) : null
        ) : !mergedProps?.sizes?.length ? null : (
          <>
            <div className="text-sm text-muted-foreground">쪽당</div>
            <Select
              value={`${pageSize}`}
